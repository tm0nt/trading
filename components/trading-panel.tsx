...
export default function TradingPanel({ tradingPair }: TradingPanelProps) {
  ...
  const handleBuyClick = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        console.log("Removendo operação e processando resultado...");
  ...
  return (
    <div className="w-80 bg-[#121212] border-l border-[#2a2a2a] flex flex-col theme-transition">
      <div className="p-4">
        ...
        <div className="border-t border-[#2a2a2a] pt-4">
          ...
          <div className="py-4">
            {activeTab === "operacoes" && (
              <div className="max-h-[300px] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-[#2a2a2a] scrollbar-track-transparent">
                {filteredOperations.length > 0 ? (
                  <div className="space-y-2">
                    {filteredOperations.map((operation) => (
                      <ActiveOperationCard key={operation.id} operation={operation} onExpire={handleOperationExpire} />
                    ))}
                  </div>
                ) : (
                  <div className="text-xs text-[#999] text-center">Não há operações abertas no momento no ativo selecionado.</div>
                )}
              </div>
            )}
                ...
                {filteredHistory.length > 0 ? (
                  <div>
                    ...
                    <div className="mt-4 pt-4 border-t border-[#2a2a2a] sticky bottom-0 bg-[#121212] pb-2">
                      <Link href="/account?section=historico" className="flex items-center justify-between w-full bg-[#1a1a1a] hover:bg-[#222] border border-[#2a2a2a] rounded-lg p-3 transition-colors">
                  ...
                  <div className="text-xs text-[#999] text-center">Não há histórico de operações para este ativo.</div>
                )}
              </div>
            )}
          </div>
